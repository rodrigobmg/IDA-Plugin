...
bool h8_jump_pattern_t::jpi_cmp_ncases()
{
  ...
  else if ( !get_op_value(&value, insn, insn.Op1) )
    return false;
...
public:
  h8_jump_pattern1_t(switch_info_t *_si)
    : h8_jump_pattern_t(_si, depends1, JPI_SUB_LOWCASE2),
      add_elbase_size(dt_void) {}

  virtual bool jpi7(void) { return jpi_sub_lowcase_tied(); }
  virtual bool jpi6(void) { return jpi_sub_lowcase(); }
  virtual bool jpi5(void) { return jpi_cmp_ncases_condjump(JPI_SHIFT); }
  virtual bool jpi4(void) { return jpi_shl(); }
  virtual bool jpi3(void) { return jpi_ld(JPI_SHIFT); }
  virtual bool jpi2(void);  // (tied to 1)
  virtual bool jpi1(void);  // add #elbase, rA
  virtual bool jpi0(void);  // jmp @rA
...
//----------------------------------------------------------------------
static int is_jump_pattern1(switch_info_t *si, const insn_t &insn)
{
  h8_jump_pattern1_t jp(si);
...
public:
  h8_jump_pattern2_t(switch_info_t *_si)
    : h8_jump_pattern_t(_si, depends2, JPI_SUB_LOWCASE2) {}

  virtual bool jpi6(void) { return jpi_sub_lowcase_tied(); }
  virtual bool jpi5(void) { return jpi_sub_lowcase(); }
  virtual bool jpi4(void) { return jpi_cmp_ncases_condjump(JPI_SHIFT); }
  virtual bool jpi3(void) { return jpi_shl(); }
  virtual bool jpi2(void) { return jpi_ld(JPI_SHIFT); }
  virtual bool jpi1(void);  // shlr.X rA
  virtual bool jpi0(void);  // bra rA
...
//----------------------------------------------------------------------
static int is_jump_pattern2(switch_info_t *si, const insn_t &insn)
{
  h8_jump_pattern2_t jp(si);
  if ( !jp.match(insn) || !jp.finish() )
    return JT_NONE;
  return JT_SWITCH;
}
