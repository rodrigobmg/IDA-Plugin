...
// set TRY_OLD_FBR if the function should look for a value at the func start
// if it failed to find the value at EA (this is the old behavior)
bool get_fbase_reg(
        fbase_reg_t *fbr,
        ea_t ea,
        func_t *pfn,  // should be get_func(ea)
        bool try_old_fbr = false);
void set_fbase_reg(ea_t ea, const fbase_reg_t &fbr, bool overwrite = true);
