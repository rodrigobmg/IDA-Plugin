...
public:
  ...
  inline void mark_switch_insns(int last = NINS - 1, int nlowcase = 0) const;
  ...
  // if <branch> and the instruction at the address <ea> has a delay
  // slot after it then update <ea> to the delay slot;
  // if not <branch> and the instruction at the address <ea> is in a
  // delay slot of a branch likely instruction then update <ea> to the
  // branch (skip the delay slot);
...
protected:
  ...
  static inline bool is_narrow(op_dtype_t dt1, op_dtype_t dt2);
  enum
  {
    dt_7bit = 255,
    dt_6bit = 254,
...
typedef int is_pattern_t(switch_info_t *si, const insn_t &insn);
...
//------------------------------------------------------------------------
inline void jump_pattern_t::mark_switch_insns(int last, int nlowcase) const
{
  // do not mark the indirect jmp (eas[0]) as ignored
  for ( uint i = 1; i <= last; ++i )
  {
    ...
    if ( ea != BADADDR && !remote_code.has(ea) )
    {
      ...
      {
        // mark will be set in set_switch_info()
        si->marks.add_unique(ea);
      }
    }
  }
  // mark insns in addition to the regular ones
  for ( uint i = 0; i < extra_insn_eas.size(); ++i )
  {
    ea_t ea = extra_insn_eas[i];
    if ( ea != eas[0] )
    {
      if ( si->get_version() == 1 )
      {
  ...
  // unmark lowcase insns
  if ( nlowcase > 0 && (si->flags & SWI_HXNOLOWCASE) != 0 )
  {
    for ( uint i = last + 1; i <= last + nlowcase; ++i )
    {
      ea_t ea = eas[i];
      if ( ea != BADADDR && is_switch_insn(ea) )
        clr_ignore_micro(ea);
    }
  }
}
        ...
        const op_t &reg) const
{
  ...
  // e.g. after 'cmp cl, imm' we can not use cx
  if ( !is_narrow(op.dtype, reg.dtype) )
    return true;
  // we believe that dword is widened to qword
  if ( modifying_r32_spoils_r64 && op.dtype == dt_dword )
    return true;
  // try to extend
  if ( !is_narrow(extend_dtype(op), reg.dtype) )
    return true;
  return false;
}
...
inline bool jump_pattern_t::is_narrow(op_dtype_t dt1, op_dtype_t dt2)
{
  if ( dt1 < dt_2bit )
    return dt2 >= dt_2bit ? false : dt1 < dt2;
  else
    return dt2 < dt_2bit ? true : dt1 < dt2;
}
...
//----------------------------------------------------------------------
inline bool jump_pattern_t::set_moved(
        const op_t &dst,
        const op_t &src,
        tracked_regs_t &_regs,
        op_dtype_t real_dst_dtype,
        op_dtype_t real_src_dtype) const
{
  if ( real_dst_dtype == dt_void )
    real_dst_dtype = dst.dtype;
  if ( modifying_r32_spoils_r64
    && dst.type == o_reg
    && real_dst_dtype == dt_dword )
  {
    real_dst_dtype = dt_qword;
  }
  if ( real_src_dtype == dt_void )
    real_src_dtype = src.dtype;

  bool changed = false;
  for ( size_t i = 0; i < _regs.size(); ++i )
  {
    if ( equal_ops(_regs[i], dst) )
    {
      op_dtype_t dst_dtype = _regs[i].dtype;
      if ( !is_narrow(real_dst_dtype, dst_dtype) )
      {
        _regs[i] = src;
        // set minimum size
        if ( is_narrow(real_src_dtype, dst_dtype) )
          dst_dtype = real_src_dtype;
        _regs[i].dtype = dst_dtype;
      }
      else
      {
        _regs[i].type = o_void;  // spoil register
...
// calculate state of registers before a conditional jump <ea> as the
// combination of states of each branch
inline void jump_pattern_t::combine_regs(
        tracked_regs_t *dst,
        const tracked_regs_t &src,
        ea_t ea)
{
