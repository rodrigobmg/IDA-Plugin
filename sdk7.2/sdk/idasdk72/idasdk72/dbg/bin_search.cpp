...
protected:
  ...
  bool sense_case;        // case-sensitive comparison?
      ...
      failed_ea(BADADDR)
  {
    for ( int i=0; i < ptns.size(); ++i )
    {
      ...
      if ( ptn.bytes.empty() )
        continue;
      if ( sense_case && ptn.all_bytes_defined() ) //TODO: && !inf.is_wide_high_byte_first() - for servers
...
public:
  forward_matcher_t(
          ...
          bool sc,
          qstring *_errbuf)
    : matcher_t(_found_ea, _mod, _ptns, sc, _errbuf),
      last_off(PAGESZ)
  {}

  //--------------------------------------------------------------------------
  virtual drc_t walk_memory_ranges(const ea_t srch_start_ea, const ea_t srch_end_ea)
  {
  ...
  drc_t find(ea_t start_ea, ea_t end_ea)
  {
        ...
        qstring *errbuf)
{
  ...
  if ( search_backward )
    matcher = new backward_matcher_t(found_ea, this, ptns, sense_case, errbuf);
  else
    matcher = new forward_matcher_t(found_ea, this, ptns, sense_case, errbuf);

  drc_t drc = matcher->walk_memory_ranges(start_ea, end_ea);
  if ( drc != DRC_ERROR )
    return drc;   //-V773 without releasing the 'matcher' pointer

  debdeb("dbg_bin_search failed to read memory at %a\n", matcher->get_failed_address());
        ...
        qstring *errbuf)
{
  ...
  if ( (srch_flags & BIN_SEARCH_BACKWARD) != 0 )
    matcher = new backward_matcher_t(found_ea, NULL, ptns, sense_case, errbuf);
  else
    matcher = new forward_matcher_t(found_ea, NULL, ptns, sense_case, errbuf);
