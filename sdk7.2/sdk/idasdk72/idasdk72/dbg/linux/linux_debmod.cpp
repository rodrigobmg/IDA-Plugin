...
//--------------------------------------------------------------------------
//lint -esym(970,long) fixme: can we use peeksize_t instead?
...
bool linux_debmod_t::del_pending_event(event_id_t id, const char *module_name)
{
  ...
  {
    if ( p->eid() == id && streq(p->modinfo().name.c_str(), module_name) )
    {
      events.erase(p);
      return true;
    }
...
void linux_debmod_t::enum_names(const char *libname)
{
  ...
  for ( easet_t::iterator p=dlls_to_import.begin(); p != dlls_to_import.end(); )
  {
    ...
    dlls_to_import.erase(p++);   // C++98
#else
    p = dlls_to_import.erase(p); // C++11
#endif
  }
}
...
#define INTEL_REG(reg) x##reg
  ...
  if ( sreg_value == regs.INTEL_REG(fs) )
    return thread_get_fs_base(tid, R_FS, pea) ? DRC_OK : DRC_FAILED;
  else if ( sreg_value == regs.INTEL_REG(gs) )
    return thread_get_fs_base(tid, R_GS, pea) ? DRC_OK : DRC_FAILED;
  else
...
#define SRCREG_XSS xss
  ...
  if ( (clsmask & (X86_RC_XMM|X86_RC_FPU|X86_RC_MMX)) != 0 )
  {
    ...
    if ( (clsmask & (X86_RC_FPU|X86_RC_MMX)) != 0 )
    {
      ...
      read_fpu_registers(
              values,
              clsmask,
              i387.st_space, sizeof(i387.st_space)/8);    //-V706 Suspicious division
  ...
  if ( (clsmask & (X86_RC_FPU|X86_RC_MMX)) != 0 )
  {
    ...
    read_fpu_registers(values, clsmask, i387.st_space, sizeof(i387.st_space)/8);    //-V706 Suspicious division
  }
#endif
  return DRC_OK;
}
...
static bool patch_reg_context(
        struct user_regs_struct *regs,
        struct user_fpregs_struct *i387,
        struct user_fpxregs_struct *x387,   //lint !e818 see macros XMM_STRUCT
        int reg_idx,
        const regval_t *value)
{
  if ( value == NULL )
  ...
  { // FPU register
    ...
    else // FPU floating point register
    {
      ...
      memcpy(fpu_float, value->fval, 10);   //lint !e669 possible data overrun  //-V512
    }
  }
...
//--------------------------------------------------------------------------
int idaapi linux_debmod_t::dbg_init(qstring * /*errbuf*/)
{
  dbg_term(); // initialize various variables
  return DBG_HAS_GET_PROCESSES | DBG_HAS_DETACH_PROCESS;
}
