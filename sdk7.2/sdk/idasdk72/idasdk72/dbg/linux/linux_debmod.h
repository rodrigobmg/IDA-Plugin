...
public:
  ...
  virtual int idaapi dbg_init(qstring *errbuf);
  virtual void idaapi dbg_term(void);
  virtual drc_t idaapi dbg_detach_process(void);
  virtual drc_t idaapi dbg_start_process(const char *path,
    const char *args,
    const char *startdir,
    int flags,
  ...
  virtual drc_t idaapi dbg_attach_process(pid_t process_id, int event_id, int flags, qstring *errbuf);
  virtual drc_t idaapi dbg_prepare_to_pause_process(qstring *errbuf);
  virtual drc_t idaapi dbg_exit_process(qstring *errbuf);
  virtual drc_t idaapi dbg_continue_after_event(const debug_event_t *event);
  virtual void idaapi dbg_stopped_at_debug_event(import_infos_t *infos, bool dlls_added, thread_name_vec_t *thr_names);
  virtual drc_t idaapi dbg_thread_suspend(thid_t thread_id);
  virtual drc_t idaapi dbg_thread_continue(thid_t thread_id);
  virtual drc_t idaapi dbg_set_resume_mode(thid_t thread_id, resume_mode_t resmod);
  virtual drc_t idaapi dbg_read_registers(thid_t thread_id,
    int clsmask,
    regval_t *values,
    qstring *errbuf);
  virtual drc_t idaapi dbg_write_register(thid_t thread_id,
    int reg_idx,
    const regval_t *value,
    qstring *errbuf);
  virtual drc_t idaapi dbg_thread_get_sreg_base(ea_t *ea, thid_t thread_id, int sreg_value, qstring *errbuf);
  virtual drc_t idaapi dbg_get_memory_info(meminfo_vec_t &areas, qstring *errbuf);
  virtual ssize_t idaapi dbg_read_memory(ea_t ea, void *buffer, size_t size, qstring *errbuf);
  virtual ssize_t idaapi dbg_write_memory(ea_t ea, const void *buffer, size_t size, qstring *errbuf);
  virtual int idaapi dbg_add_bpt(bytevec_t *orig_bytes, bpttype_t type, ea_t ea, int len);
...
#ifdef __ARM__
  virtual void adjust_swbpt(ea_t *p_ea, int *p_len);
#endif
