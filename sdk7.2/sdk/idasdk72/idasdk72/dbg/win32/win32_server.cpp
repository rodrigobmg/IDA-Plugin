    ...
    case WIN32_IOCTL_PDB_OPEN:
      {
        ...
        pdbargs_t args;
        args.pdb_path = extract_cstr(&ptr, end);
        args.input_path = extract_cstr(&ptr, end);
        extract_memory(&ptr, end, &args.pdb_sign, sizeof(args.pdb_sign));
        args.spath = extract_cstr(&ptr, end);
        args.loaded_base = unpack_ea64(&ptr, end);
        args.flags = unpack_dd(&ptr, end);
    ...
    case WIN32_IOCTL_PDB_FETCH_CHILDREN:
      {
        ...
        DWORD sym_id = unpack_dd(&ptr, end);
        // msg("Fetch%s 0x%x\n",
        ...
        {
          // Children
          enum SymTagEnum children_type = (enum SymTagEnum)unpack_dd(&ptr, end);
    ...
    case WIN32_IOCTL_PDB_SIP_FETCH_LINES_BY_VA:
      {
        ...
        const uchar *end = ptr + size;
        ENSURE_SESSION_OPENED();
        ea_t va = unpack_ea64(&ptr, end);
        uint64 length = unpack_dq(&ptr, end);
    ...
    case WIN32_IOCTL_PDB_SIP_FETCH_LINES_BY_COORDS:
      {
        ...
        DWORD file_id = unpack_dd(&ptr, end);
        DWORD lnnum = unpack_dd(&ptr, end);
        DWORD colnum = unpack_dd(&ptr, end);
    ...
    case WIN32_IOCTL_PDB_SIP_FETCH_SYMBOLS_AT_VA:
      {
        ...
        ea_t va = unpack_ea64(&ptr, end);
        uint64 length = unpack_dq(&ptr, end);
        enum SymTagEnum type = (enum SymTagEnum) unpack_dd(&ptr, end);
    ...
    case WIN32_IOCTL_PDB_SIP_FIND_FILES:
      {
        ...
        qstring fname = extract_cstr(&ptr, end);
